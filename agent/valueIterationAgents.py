        ...
        for iter in range(self.iterations):
            ...
            for s in self.mdp.getStates():
                if not self.mdp.isTerminal(s):
                    ...
                    tempValues[s] = self.getQValue(s, action)
...
class AsynchronousValueIterationAgent(ValueIterationAgent):
    ...
    def runValueIteration(self):
        ...
        for i in range(self.iterations):
            if stateIndex == len(mdpStates):
                stateIndex = 0
            targetState = mdpStates[stateIndex]
            stateIndex += 1
            if self.mdp.isTerminal(targetState):
                continue
            bestAction = self.getAction(targetState)
            QValue = self.getQValue(targetState, bestAction)
